...
- `glbtn_key=all` or `glbtn_gpio=all`
  → Scan all UCLASS_BUTTONs; any button press will trigger the GPIO.
- `glbtn_gpio=<gpio>`
...
- Scan gpio-keys:
  `setenv glbtn_key all`
  > wps、reset、mesh、all...

### Disable auto-reboot after upgrade

Set failsafe_auto_reboot environment variable to 0/false/no/off to disable auto reboot after upgrade.
